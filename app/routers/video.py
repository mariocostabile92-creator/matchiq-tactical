...
def _public_video_report(row: dict):
    ...
    return {
        ...
        "frame_times": payload.get("frame_times") or [],
        "frame_meta": payload.get("frame_meta") or [],
        "tactical_lines": payload.get("tactical_lines") or [],
    ...
    if data.video_asset_id and cloud_save and cloud_save.get("success") and not cloud_save.get("deduplicated"):
        record_video_session_activity(
            ...
            latest_report={
                ...
                "frames": len(frames),
                "focus": _clean_text(data.focus, 160),
                "frame_times": data.frame_times[:12],
                "frame_meta": data.frame_meta[:12],
                "tactical_lines": data.tactical_lines[:12],
                "slides_count": len((slides or {}).get("slides", []) or []),
                "deck_summary": _clean_text((slides or {}).get("deck_summary", ""), 220),
        ...
        metadata={
            ...
            **storage_descriptor(saved),
            ...
            "activity": [{
                ...
                "details": "File salvato nella Libreria Video AI.",
                "at": now,
            }],
        },
    )
    ...
    if data.video_asset_id and not result.get("deduplicated"):
        record_video_session_activity(
            ...
            latest_report={
                ...
                "focus": _clean_text(data.focus, 160),
                "frame_times": (data.payload or {}).get("frame_times", [])[:12] if isinstance(data.payload, dict) else [],
                "frame_meta": (data.payload or {}).get("frame_meta", [])[:12] if isinstance(data.payload, dict) else [],
                "tactical_lines": (data.payload or {}).get("tactical_lines", [])[:12] if isinstance(data.payload, dict) else [],
                "slides_count": len((data.payload or {}).get("slides", []) or []) if isinstance(data.payload, dict) else 0,
                "deck_summary": _clean_text((data.payload or {}).get("deck_summary", ""), 220) if isinstance(data.payload, dict) else "",
                "created_at": datetime.utcnow().isoformat(),
            },
        )
