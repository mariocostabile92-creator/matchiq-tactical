...
const CACHE_TOP = "matchiq_live_matches_top_v4";
const CACHE_ALL = "matchiq_live_matches_all_v4";
const CACHE_LAST_VALID = "matchiq_last_valid_live_matches_v4";

const FINISHED_STATUSES = ["FT", "AET", "PEN", "CANC", "PST", "ABD", "AWD", "WO"];
...
function getSafeModeLabel() {
    if (API_USAGE_PERCENT < 50) return "🟢 NORMAL MODE";
    if (API_USAGE_PERCENT < 70) return "🟡 SMART SAFE MODE";
    if (API_USAGE_PERCENT < 85) return "🟠 API SAFE MODE";
    if (API_USAGE_PERCENT < 95) return "🔴 CRITICAL SAFE MODE";
...
function getScore(match) {
    return match.score || `${match.home_goals ?? 0}-${match.away_goals ?? 0}`;
}
...
function isFinishedMatch(match) {
    ...
    if (minute >= 100 && status !== "LIVE" && status !== "1H" && status !== "2H") {
        return true;
    }
...
function saveCache(matches) {
    try {
        const cleanMatches = normalizeMatches(matches);
        ...
        localStorage.setItem(CACHE_LAST_VALID, JSON.stringify(payload));
    } catch (e) {
...
function loadCache() {
    try {
        const raw = localStorage.getItem(getCacheKey()) || localStorage.getItem(CACHE_LAST_VALID);
        if (!raw) return null;

        const parsed = JSON.parse(raw);
        const ageSeconds = (Date.now() - Number(parsed.timestamp || 0)) / 1000;

        if (ageSeconds > 240) return null;

        return normalizeMatches(parsed.matches || []);
    } catch {
        return null;
    }
}
...
function clearOldCache() {
    try {
        ...
        localStorage.removeItem("matchiq_live_matches_top_v2");
        localStorage.removeItem("matchiq_live_matches_all_v2");
        localStorage.removeItem("matchiq_last_valid_live_matches_v2");
    } catch {}
}
...
function createMatchCard(match) {
    ...
    const status = getMatchStatus(match);
    const isMemory = Boolean(match.memory_mode);

    const card = document.createElement("div");
    card.className = "match-card live-card-enter";
    card.dataset.matchId = matchId;
    card.dataset.signature = matchSignature(match);
    ...
    card.innerHTML = `
        ...
        <div class="teams-row">
            <div class="team-box">
                <img class="team-logo" src="${match.home_logo || ""}" alt="${match.home || "Home"}">
                <div class="team-name">${match.home || "Home"}</div>
            </div>

            <div class="score-box">
                <div class="score" data-role="score">${score}</div>
                <div class="minute" data-role="minute">${minute}'</div>
                <div class="mini-status">${status || "LIVE"}</div>
            </div>

            <div class="team-box">
                <img class="team-logo" src="${match.away_logo || ""}" alt="${match.away || "Away"}">
                <div class="team-name">${match.away || "Away"}</div>
            </div>
        </div>

        <div class="match-footer">
            <div class="live-label ${isMemory ? "memory-label" : ""}" data-role="liveLabel">
                ${isMemory ? "● LIVE MEMORY" : "● LIVE"}
            </div>
            <button class="analysis-btn" onclick="openAnalysis('${matchId}')">
                OPEN ANALYSIS
            </button>
        </div>
    `;
    ...
    setTimeout(() => {
        card.classList.remove("live-card-enter");
    }, 500);
...
function updateMatchCard(card, oldMatch, newMatch) {
    ...
    const minuteEl = card.querySelector('[data-role="minute"]');
    const liveLabelEl = card.querySelector('[data-role="liveLabel"]');
    const statusEl = card.querySelector(".mini-status");

    if (scoreEl && oldScore !== newScore) {
        scoreEl.textContent = newScore;
        card.classList.add("score-changed");
        setTimeout(() => card.classList.remove("score-changed"), 1200);
    }

    if (minuteEl && oldMinute !== newMinute) {
        minuteEl.textContent = `${newMinute}'`;
        minuteEl.classList.add("minute-pulse");
        setTimeout(() => minuteEl.classList.remove("minute-pulse"), 900);
    }
    ...
    if (liveLabelEl && wasMemory !== isMemory) {
        liveLabelEl.textContent = isMemory ? "● LIVE MEMORY" : "● LIVE";
        liveLabelEl.classList.toggle("memory-label", isMemory);
    }

    card.dataset.signature = matchSignature(newMatch);
    card.classList.add("live-updated");
    setTimeout(() => card.classList.remove("live-updated"), 800);
}

function renderMatchesSmart(matches) {
    const liveMatches = normalizeMatches(matches);
    ...
    const emptyState = matchesContainer.querySelector(".empty-state");
    if (emptyState) matchesContainer.innerHTML = "";
    ...
    liveMatches.forEach(match => {
        ...
        const existing = matchesContainer.querySelector(`[data-match-id="${matchId}"]`);
        const previous = currentMatchesMap.get(matchId);
        ...
        if (previous && matchSignature(previous) !== matchSignature(match)) {
...
function injectSmartStyles() {
    if (document.getElementById("matchiq-smart-refresh-styles")) return;

    const style = document.createElement("style");
    style.id = "matchiq-smart-refresh-styles";
...
async function loadMatches({ useLoading = false, force = false } = {}) {
    if (isLoading) return;
    ...
    if (API_USAGE_PERCENT >= 95 && !force) {
        if (cached && cached.length > 0) renderMatchesSmart(cached);
        else setEmpty();
        return;
    }
    ...
    try {
        const response = await fetch(`${API}/api/live-matches?top_only=${topOnly}&t=${Date.now()}`, {
            cache: "no-store"
        });
    ...
    []
        ...
        console.error("Errore caricamento live matches:", error);
...
function setupButtons() {
    topBtn.onclick = () => {
        ...
        resetView();
        loadMatches({ useLoading: true, force: true });
        startSmartRefresh();
    };
